...
{project-name} streamlines the generation of SBOMs using {cyclonedx-maven-plugin-link}.
Plugin execution generates SBOM files for each module, including the root one.
Generated SBOM files are attached as artifacts with `cyclonedx` classifier and `xml`/`json`  extensions, that is, `<artifactId>-<version>-cyclonedx.{xml,json}`.
