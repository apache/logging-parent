...
If the `npm-dependencies-ref` input is provided, the NPM dependencies are downloaded from that branch or tag of {project-github-url}[`logging-parent`] (or of the repository given in `npm-dependencies-repository`).
Otherwise the `package.json` committed in the repository is used.
